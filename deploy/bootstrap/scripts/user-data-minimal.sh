...
runcmd:
  - |
    ...
    if command -v ossutil >/dev/null 2>&1; then
      log "使用 ossutil 下载脚本..."
      ossutil cp "oss://${oss_bucket_name}/$SCRIPT_PATH" /tmp/k3s-init.sh || {
        log "⚠️  ossutil 下载失败，尝试 HTTP 下载..."
        # 如果 ossutil 失败，尝试 HTTP 下载（需要 Bucket 是公开的或使用签名 URL）
        curl -f "https://${oss_bucket_name}.oss-${oss_region}.aliyuncs.com/$SCRIPT_PATH" -o /tmp/k3s-init.sh || {
